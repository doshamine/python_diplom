...
def test_product_filter_by_price_and_shop(api_client, product):
    ...
    baker.make('backend.ProductInfo', product=product, price=200, shop=product.product_info.first().shop)
    baker.make('backend.ProductInfo', product=product2, price=100, shop=shop2)
    url = reverse('products-list')

    response = api_client.get(url, {'min_price': 150})
    assert response.status_code == 200
    assert all(item['id'] == product.id for item in response.data)
